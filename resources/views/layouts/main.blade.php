...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <title>{{ __('Gimnasio') }}</title>
    <link rel="img_prueba" sizes="76x76" href="{{ asset('img/img_prueba.png') }}">
    <link rel="icon" type="image/png" href="{{ asset('img/img_prueba.png') }}">
    ...
    <body class="{{ $class ?? '' }}">
        @auth()
            ...
            @include('layouts.page_templates.auth')<!--auth==logeado-->
        @endauth
        @guest()
            @include('layouts.page_templates.guest')<!--guest==no logeado-->
        @endguest
        @if (auth()->check())
